...
public class SetBudget extends Command {
    ...
    public String execute(User user, String message) {
        boolean operationResult = user.setMonthBudget(Float.parseFloat(message));
        if (operationResult) {
            EntityManager.updateUser(user);
            return String.format("Отлично, Вы установили ваш " + "ежемесячный бюджет. Он составляет %s рублей", user.checkMonthBudget());
        }
